...
import { HIDDEN_CLASS } from "../types";

export function setupMutationObserver(plugin: SimpleFocusPlugin): void {
	// Watch for new tree items being added to the DOM (virtual scrolling)
	plugin.mutationObserver = new MutationObserver((mutations) => {
				...
				if (node.nodeType === Node.ELEMENT_NODE) {
					...
					for (const treeItem of Array.from(treeItems)) {
						...
						if (pathElement) {
							...
							if (path && !path.startsWith(plugin.focusedPath!) && path !== plugin.focusedPath) {
								// This item should be hidden
								if (!path.startsWith(plugin.focusedPath! + '/')) {
									treeItem.addClass(HIDDEN_CLASS);
								}
							}
						}
					}
	...
	if (fileExplorerLeaves.length > 0) {
		...
		if (navFilesContainer) {
			plugin.mutationObserver.observe(navFilesContainer, {
				childList: true,
				subtree: true
			});
		}
