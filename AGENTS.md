...
- **Architecture**: Organized structure - main code in `src/main.ts`, settings in `src/ui/settings-tab.ts`, utilities in `src/utils/`
- **Purpose**: Plugin that allows focusing on specific files/folders in the file explorer to reduce visual clutter
- **Functionality**: Hides other files when in focus mode, showing only the focused item, its children, and ancestors
- **Status**: Production (forked from Simple Focus, being improved with modern patterns)

### Important Project-Specific Details

- **Project Type**: Plugin
- **Purpose**: Reduce visual clutter in file explorer by focusing on specific files/folders
- **Status**: Production (forked from Simple Focus, being improved)
- **Key Features**: 
  - Multiple focus modes (file, parent, grandparent, great-grandparent, custom folder)
  - Right-click menu integration for quick focusing
  - File explorer icon button for easy toggling
  - Hide ancestor folders option
  - Mobile and desktop support
- **Architecture**: 
  - Organized structure with main code in `src/main.ts`
  - Settings UI in `src/ui/settings-tab.ts` using SettingGroup compatibility
  - Utilities in `src/utils/` for file explorer patching, icon management, and focus path calculation
  - Multi-language support via `lang/` directory (English, Chinese)

### Maintenance Tasks

- Keep reference repos updated (Simple Focus original, UI Tweaker for patterns)
- Monitor Obsidian API changes for SettingGroup compatibility
- Test on both desktop and mobile platforms
- Update language files when adding new features

### Project-Specific Conventions

- Uses SettingGroup compatibility utility (`src/utils/settings-compat.ts`) for backward compatibility with Obsidian 1.11.0+
- File explorer patching for visibility control (patches `getSortedFolderItems` method)
- Multi-language support via `lang/` directory with English and Chinese translations
- CSS classes use `explorer-focus-*` prefix
- Data attributes use `data-explorer-focus-hidden` for hidden file items
...
- `src/main.ts` - Plugin entry point, lifecycle management, focus logic, file explorer icon management
- `src/ui/settings-tab.ts` - Settings UI using SettingGroup compatibility utility
- `src/utils/settings-compat.ts` - SettingGroup backward compatibility utility (supports Obsidian 1.11.0+ with fallback)
- `src/utils/file-explorer-patch.ts` - File explorer patching for visibility control (patches `getSortedFolderItems`)
- `src/utils/file-explorer.ts` - File explorer icon creation and management, navigation button container finding
- `src/utils/focus.ts` - Focus path calculation utilities (determines focus path based on level)
- `src/types.ts` - TypeScript interfaces (`ExplorerFocusSettings`) and default settings
- `src/commands/index.ts` - Command registration (toggle focus command, right-click menu integration)
- `lang/en.ts` and `lang/zh.ts` - Language files for English and Chinese translations

### Development Notes

- Uses SettingGroup compatibility pattern for Obsidian 1.11.0+ with fallback for older versions
- Patches file explorer view to control file visibility by modifying `getSortedFolderItems` method
- Supports both desktop and mobile platforms with responsive icon placement
- Multi-language support via `lang/` directory structure
- File explorer icon is dynamically inserted into the navigation buttons container
- Focus mode shows focused item, its children, and ancestors while hiding everything else
- Uses CSS classes (`explorer-focus-mode`) and data attributes (`data-explorer-focus-hidden`) for styling and state management
